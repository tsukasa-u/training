...
function main()
    ...
    funcs.ge = (x, u) -> begin
        ...
        # ]
        return [0.0]
        # return [ u_r_p^2*u_r_m^2, u_θ_p^2*u_θ_m^2]
    end
    funcs.gef = (x) -> [0.0]
    # funcs.gl = (x, u) -> begin
    #     r, v_r, v_θ = x
    #     u_r_p, u_r_m, u_θ_p, u_θ_m = u
    #     # return exp.([u_r_p + u_r_m - 0.01, u_θ_p + u_θ_m - 0.01, -u_r_p - u_r_m, -u_θ_p - u_θ_m, u_r_p - u_r_m - 0.01, u_θ_p - u_θ_m - 0.01, -(u_r_p - u_r_m) - 0.01, -(u_θ_p - u_θ_m) - 0.01]).-1.0
    #     # return [u_r_p + u_r_m - β*0.01, u_θ_p + u_θ_m - β*0.01, -(u_r_p + u_r_m), -(u_θ_p + u_θ_m), u_r_p - u_r_m - β*0.01, u_θ_p - u_θ_m - β*0.01, -(u_r_p - u_r_m) - β*0.01, -(u_θ_p - u_θ_m) - β*0.01]
    #     return [u_r_p - β*0.01, u_θ_p - β*0.01, u_r_m - β*0.01, u_θ_m - β*0.01]*10.0
    # end
    funcs.glf = (x) -> [0.0]

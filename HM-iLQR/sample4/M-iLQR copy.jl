        ...
        for α in [2.0^(-k) for k in 3:10]
            ...
            for i in 1:U.M
                for j in (1:U._L)
                    ...
                    # println("U[i, j+1, :] = ", U[i, j+1, :])
                    # println("U.limit_lower = ", U.limit_lower)
                    if any(X.limit_lower .> X[i, j+1, :]) || any(X.limit_upper .< X[i, j+1, :])
                        failed = true
                        break
                    end
                    if any(U.limit_lower .> U[i, j, :]) || any(U.limit_upper .< U[i, j, :])
                        failed = true
                        break
                    end
                end

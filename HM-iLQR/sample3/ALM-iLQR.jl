...
module ALM_iLQR
    ...
    function forwardPass(_X, _U, k, K, k__, K__, d, funcs, α::Float64)
        X = copy(_X)
        U = copy(_U)
        for i in 1:U.M
            for j in (1:U._L)
                ...
                __du = α*k[i, j, :] + K[i, j, :, :] * __dx
                __U = min.(max.(_U_ + __du, U.limit_lower), U.limit_upper)
                ___du = __U - _U_

                __X = _X[i, j+1, :] + funcs.fx(_X_, _U_) * __dx + funcs.fu(_X_, _U_) * ___du + d[i, j]
                __X = min.(max.(__X, X.limit_lower), X.limit_upper)


                __du__ = α*k__[i, j, :] + K__[i, j, :, :] * __dx
                __U__ = min.(max.(_U_ + __du__, U.limit_lower), U.limit_upper)
                ___du__ = __U__ - _U_

                __X__ = _X[i, j+1, :] + funcs.fx(_X_, _U_) * __dx + funcs.fu(_X_, _U_) * ___du__ + d[i, j]
                __X__ = min.(max.(__X__, X.limit_lower), X.limit_upper)

                # __du = α*k[i, j, :] + K[i, j, :, :] * __dx
                # U[i, j, :] = min.(max.(_U_ + __du, U.limit_lower), U.limit_upper)
                # ___du = U[i, j, :] - _U_

                # X[i, j+1, :] = _X[i, j+1, :] + funcs.fx(_X_, _U_) * __dx + funcs.fu(_X_, _U_) * ___du + d[i, j]

                # X[i, j+1, :] = min.(max.(X[i, j+1, :], X.limit_lower), X.limit_upper)

                if any(funcs.gl(__X, __U) .> 0)
                    X[i, j+1, :] = __X__
                    U[i, j+1, :] = __U__
                else
                    X[i, j+1, :] = __X
    ...
    M_iLQR.wrapforwardPass(X, U, k, K, k__, K__, d, funcs, α) = forwardPass(X, U, k, K, k__, K__, d, funcs, α)

    function RunM_iLQR(X_init, U_init, μ, μf, λ, λf, N, M, MaxIter, ϵ_v, d_max, funcs, α)
        ...
        Vx__, Vxx__ = M_iLQR.initV__(X, funcs)
        ...
        for idx in 1:MaxIter
            ...
            M_iLQR.updateV__!(X, Vx, Vxx, funcs)
            
            k, K, ΔV = M_iLQR.wrapbackwardPass!(X, U, λ, μ, Vx, Vxx, d, funcs)
            k__, K__, ΔV__ = M_iLQR.wrapbackwardPass!(X, U, λ, μ, Vx__, Vxx__, d, funcs)

            X, U = M_iLQR.wrapforwardPass(X, U, k, K, d, funcs, α)
            ...
            plot_X = plot(X[:, :, 1], label="1 $(idx) : ΔJ = $(_J)")
            plot!(plot_X, X[:, :, 2], label="2")
            plot!(plot_X, X[:, :, 3], label="3")

            plot_U = plot(U[:, :, 1]-U[:, :, 2], label="1-2")
            plot!(plot_U, U[:, :, 3]-U[:, :, 4], label="3-4")
            plot!(plot_U, U[:, :, 1]+U[:, :, 2], label="1+2")
            plot!(plot_U, U[:, :, 3]+U[:, :, 4], label="3+4")

            plot_Z = plot(U[:, :, 1], label="1")
            plot!(plot_Z, U[:, :, 2], label="2")
            plot!(plot_Z, U[:, :, 3], label="3")
            plot!(plot_Z, U[:, :, 4], label="4")

            frame(
                anim, 
                plot(plot_X, plot_U, plot_Z, layout=(3, 1))
            )

            if (abs(ΔJ) < ϵ_v && norm(d, 2) < d_max) || idx == MaxIter
            # if idx == MaxIter
                gif(anim, "ALM-iLQR_anim_fps60.gif", fps = 60)
                return X, U, K
            end
    ...
    M_iLQR.wrapRunM_iLQR(X_init, U_init, μ, μf, λ, λf, N, M, MaxIter, ϵ_v, d_max, funcs, α) = RunM_iLQR(X_init, U_init, μ, μf, λ, λf, N, M, MaxIter, ϵ_v, d_max, funcs, α)
    ...
    function RunALM_iLQR(X_init, U_init, N, M, MaxIter, ϵ_v, d_max, funcs, α)

        λ, λf, μ, μf, φ, ϵ_g = init(U_init._L, U_init.M, U_init.N, size(funcs.h(X_init[1, 1, :], U_init[1, 1, :])), size(funcs.hf(X_init[1, 1, :])))
        
        X, U, K = M_iLQR.wrapRunM_iLQR(X_init, U_init, μ, μf, λ, λf, N, M, MaxIter, ϵ_v, d_max, funcs, α)
        
        λ, λf, μ, μf = update_λ_μ(λ, λf, μ, μf, X, U, funcs, φ)

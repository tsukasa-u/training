...
module M_iLQR
    ...
    function forwardPass(_X, _U, k, K, d, funcs, α::Float64)
    # function forwardPass(_X, _U, k, K, d, funcs, α, _dx)
        ...
        for i in 1:U.M
            for j in (1:U._L)
                ...
                U[i, j, :] = min.(max.(_U[i, j, :] + __du, U.limit_lower), U.limit_upper)
                ___du = U[i, j, :] - _U[i, j, :]

                X[i, j+1, :] = _X[i, j+1, :] + funcs.fx(_X[i, j, :], _U[i, j, :]) * __dx + funcs.fu(_X[i, j, :], _U[i, j, :]) * ___du + d[i, j]
    ...
    wrapforwardPass(_X, _U, k, K, d, funcs, α::Float64) = forwardPass(_X, _U, k, K, d, funcs, α::Float64)
    ...
    function initV__(X, funcs)
        vx__ = Main.func.Marray(X._L, X.M, X.N, (X.n[1],))
        Main.func.setEndMarray!(vx__, funcs.lfx__(Main.func.getEndMarray(X, [:])), [:])
        vxx__ = Main.func.Marray(X._L, X.M, X.N, (X.n[1], X.n[1]))
        Main.func.setEndMarray!(vxx__, funcs.lfxx__(Main.func.getEndMarray(X, [:])), [:, :])
        return vx__, vxx__
    end
    ...
    function updateV__!(X, vx__, vxx__, funcs)
        Main.func.setEndMarray!(vx__, funcs.lfx__(Main.func.getEndMarray(X, [:])), [:])
        Main.func.setEndMarray!(vxx__, funcs.lfxx__(Main.func.getEndMarray(X, [:])), [:, :])
    end
    ...
    function RunM_iLQR(X_init, U_init, N, M, MaxIter, ϵ_v, d_max, funcs)
        ...
        # Vx__, Vxx__ = initV__(X, funcs)

        for idx in 1:MaxIter
            println("start step ", idx, ", J = ", _J)

            updateV!(X, Vx, Vxx, funcs)
            # updateV__!(X, Vx__, Vxx__, funcs)

            k, K, ΔV = wrapbackwardPass(X, U, Vx, Vxx, d, funcs)
            # k__, K__, ΔV__ = wrapbackwardPass!(X, U, Vx__, Vxx__, d, funcs)

            X, U = wrapforwardPass!(X, U, k, K, d, funcs, 1.0)

...
module ALM_iLQR
    ...
    function compute_J1(X, U, λ, μ, funcs)
        a = (X, U, λ, μ, funcs) -> λ'*funcs.h(X, U) + 0.5*μ*norm(funcs.h(X, U))^2
        # return Main.func.sumMarray(a, X, U, λ, μ, h) + Main.func.endMarray(a, X, λ, μ, h)
        return M_iLQR.compute_J(X, U, funcs) + Main.func.sumMarray(a, X, U, λ, μ) + Main.func.endMarray(a, X, λ, μ)
    end
    ...
    function backwardPass!(X, U, λ, μ, Vx, Vxx, d, funcs)
        ...
        for i in U.M:-1:1
            for j in U._L:-1:1
                ...
                Qx = funcs.lx(X[i, j, :], U[i, j, :]) + funcs.fx(X[i, j, :], U[i, j, :])' * Vx_ + funcs.hx(X[i, j, :], U[i, j, :])'*(λ[i, j, :] + μ[i, j, :]*funcs.h(X[i, j, :], U[i, j, :]))
                Qu = funcs.lu(X[i, j, :], U[i, j, :]) + funcs.fu(X[i, j, :], U[i, j, :])' * Vx_ + funcs.hu(X[i, j, :], U[i, j, :])'*(λ[i, j, :] + μ[i, j, :]*funcs.h(X[i, j, :], U[i, j, :]))
                Qxx = funcs.lxx(X[i, j, :], U[i, j, :]) + funcs.fx(X[i, j, :], U[i, j, :])' * Vxx[i, j+1, :, :] * funcs.fx(X[i, j, :], U[i, j, :]) + μ[i, j, :]*funcs.hx(X[i, j, :], U[i, j, :])'*funcs.hx(X[i, j, :], U[i, j, :])
                Quu = funcs.luu(X[i, j, :], U[i, j, :]) + funcs.fu(X[i, j, :], U[i, j, :])' * Vxx[i, j+1, :, :] * funcs.fu(X[i, j, :], U[i, j, :]) + μ[i, j, :]*funcs.hu(X[i, j, :], U[i, j, :])'*funcs.hu(X[i, j, :], U[i, j, :])
                Qux = funcs.lux(X[i, j, :], U[i, j, :]) + funcs.fu(X[i, j, :], U[i, j, :])' * Vxx[i, j+1, :, :] * funcs.fx(X[i, j, :], U[i, j, :]) + μ[i, j, :]*funcs.hu(X[i, j, :], U[i, j, :])'*funcs.hx(X[i, j, :], U[i, j, :])
                k[i, j, :] = -Quu \ Qu
                K[i, j, :, :] = -Quu \ Qux
                ΔV[i, j] = 0.5 * Qu' * k[i, j, :]
    ...
    wrapRunM_iLQR(X_init, U_init, μ, λ, N, M, MaxIter, ϵ_v, d_max, funcs) = RunM_iLQR(X_init, U_init, μ, λ, N, M, MaxIter, ϵ_v, d_max, funcs)
    ...
    function init(_L, M, N, n)
        return Main.func.onesMarray(0_L, M, N, vec(n)), Main.func.onesMarray(_L, M, N, vec(n)), 2.0, 1E-6
    end

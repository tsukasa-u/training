...
int main() {
    ...
    compute_D((void*)d, length_n, 0.0, 0.0);
    ...
    for (unsigned int i = 2*(length_n + 1) + 1; i < 3*(length_n + 1); ++i) {
        datas[i - 2*(length_n + 1) - 1] = (my_constraint_data_b){i, 1.0};
        nlopt_add_inequality_constraint(opt, myconstraint_b, &datas[i - 2*(length_n + 1) - 1], 1e-8);
    }
    ...
    for (int i = 0; i <= 3*(length_n + 1); ++i) x[i] = 0.5*((double)i/(double)length_n);
    double minf;

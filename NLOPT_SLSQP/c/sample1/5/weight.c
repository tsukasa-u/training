...
double compute_Chebyshev(unsigned int n, double x) {
    ...
    // printf("n%f\n", (pow(x + r, (double) n) + pow(x - r, (double) n))/2.0);
    // return (pow(x + r, (double) n) + pow(x - r, (double) n))/2.0;
    double theta = acos(x);
    // printf("f%f %f\n", x, theta);
    return cos((double)n*theta);
}

double compute_tau(unsigned int n, unsigned int k) {
    // printf("n%f\n", cos(((double)k + 0.5)*M_PI/((double)k)));
    // printf("g%d %f %f\n", k, ((double)k + 0.5)/((double)n), -cos(((double)k + 0.5)*M_PI/((double)n)));
    return -cos(((double)k)*M_PI/((double)n));
}
...
//         w[0] = double_alpha_p*gamma_1_2/(N_2*gamma_N*gamma_2*p_2);
//     }
...
//         double x = 1.0;
//         double p_2 = pow(compute_Chebyshev(n, x), 2.0);
//         w[n] = double_alpha_p*gamma_1_2/(N_2*gamma_N*gamma_2*p_2);
//     }
// }

void compute_weight_Chebyshev(double* w, unsigned int n) {
    
// i = 0..(n+1)
    for (unsigned int i = 0; i <= n; ++i) {
        w[i] = 2.0/((double)n + 1.0);
    }
}

void compute_D(void* d, unsigned int n, double alpha, double beta, double* tau) {
    ...
    for (unsigned int j = 1; j < n - 1; ++j) for (unsigned int k = j + 1; k < n; ++k) (*d_p)[k][j] = p_n[k]/p_n[j]/(tau[k] - tau[j]);
    ...
    for (unsigned int k = 1; k < n; ++k) (*d_p)[k][0] = p_n[k]/p_n[0]*(beta + 1.0)/(1.0 + tau[k]);
    for (unsigned int k = 1; k < n; ++k) (*d_p)[k][n] = -p_n[k]/p_n[n]*(alpha + 1.0)/(1.0 - tau[k]);

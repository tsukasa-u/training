...
function solve_NLP(
    ...
    JuMP.@NLexpressions(
        ...
        begin
            ...
            # wf, sum(W[n[i]][k]*int_expr[i, k] for i in 1:ns for k in 1:n[i])
        end
    )
    ...
    int_expr = JuMP.Containers.SparseAxisArray(
        Dict(
            ...
            for i in 1:ns for k in 1:n[i]
        )
    )

    JuMP.@NLexpression(rocket, wf, sum(W[n[i]][k]*int_expr[i, k] for i in 1:ns for k in 1:n[i]))
    ...
    if (ns >= 1)
        # JuMP.@NLconstraint(rocket,con3[j = 1:nx],  _x[ns, j, n[ns] + 1] - _x[ns, j, 1] == xDw[ns, j])
        if (ns > 1)
            # JuMP.@NLconstraint(rocket, con2[i = 2:ns, j = 1:nx], _x[i, j, 1] - _x[i - 1, j, 1] == xDw[i, j])
...
function main()
    ...
    n_max = 50
    n_min = 1
    τ = Array{Real, 1}[compute_tau(n, -0.5, -0.5) for n in n_min:n_max]
    ...
    while (true)
        index += 1
        (x, u, dx, fx) = solve_NLP(ns, nx, nu, n, x, cx, u, cu, bx, bu, c_eq, c_le, f, na, a, f_obj, na_obj, a_obj, f_int, na_int, a_int, τ, t0, tf, D, W)

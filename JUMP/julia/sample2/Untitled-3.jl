...
function solve_NLP(
        ...
        bound_x::Tuple{Array{Real, 2}},
        u::Tuple{Array{Real, 2}}, 
        bound_u::Tuple{Array{Real, 2}},
        τ::Tuple{Array{Real, 1}},
        t0::Array{Real, 1},
        tf::Array{Real, 1},
        ...
        @variables( rocket, bound_x[1][i][j, k] ≥ _x[i = 1:ns][j = 1:nx, k = 1:(n[i] + 1)] ≥ bound_x[0][i][j, k] )
        @variables( rocket, bound_u[1][i][j, k] ≥ _u[i = 1:ns][j = 1:nu, k = 1:(n[i] + 1)] ≥ bound_u[0][i][j, k] )
        ...
        @NLconstraint(rocket, [i = 1:ns][j = 1:nx, k = 1:(n[i] + 1)] == T_2[i]*_x[i][j, k])

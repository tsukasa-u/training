...
function compute_η(
        ...
        return sum(abs(dx[k] - T_2*f[k])*w[k] for k in 1:n)
end
...
function compute_ε(
        ...
        return ([compute_η(n[i], dx[i], f[i], W[n[i]], T_2[i])/w for i in 1:ns], w)
end
...
function refine(
        ...
        for j in 1:nx
            tmp = compute_ε(
                ...
                W,
                Real[(tf[i] - t0[i])/2 for i in 1:ns]
            )
            push!(εx, tmp[1])
            push!(wx, tmp[2])
        end
...
function save_file(plot_x, plot_u, plot_t)
    ...
    open("file.txt","w") do out
        println(out, "idx,t,x1,x2,x3,u1,u2,u3,u4")

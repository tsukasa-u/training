...
function compute_w(
        ...
        return maximum([
            max(
                ...
                maximum([
                    abs((x[i][k + 1] - x[i][k])/(t[i][k + 1] - t[i][k])) 
                    for k in 1:n[i]
                ])
            ) 
            for i in 1:ns
...
function solve_NLP(
    ...
    JuMP.set_attribute(rocket, "tol", 1e-8)
    ...
    @NLobjective(rocket, Min, obj_expr + wf)
...
function main()
    ...
    W = Array{Real, 1}[compute_W(n, alpha, beta, τ[n]) for n in n_min:n_max]
    D = Array{Real, 2}[compute_D(n, alpha, beta, τ[n]) for n in n_min:n_max]
    ...
    cu = Tuple{Real, Real}[(0.01, -0.01) for j in 1:nu]
    ct = (55.0, 0.0)
    bt = (0.0, 55.0)
    bx = Tuple{Real, Real}[
        (1.0, 4.0),
    ...
    f = Function[
        ((nx, nu, na, t, r, v_r, v_t, u_r, u_t) -> (v_r)),
        ((nx, nu, na, t, r, v_r, v_t, u_r, u_t) -> (v_t^2.0/r - 1.0/r^2.0 + u_r)),
        ((nx, nu, na, t, r, v_r, v_t, u_r, u_t) -> (-v_r*v_t/r +u_t))
    ]
    a = Real[]
    f_obj = (nx, nu, na, t, r, v_r, v_t, u_r, u_t) -> 0.0
    na_obj = 0
    a_obj = Real[]
    f_int = (nx, nu, na, t, r, v_r, v_t, u_r, u_t) -> abs(u_r) + abs(u_t)
    na_int = 0
    a_int = Real[]
    t0 = Real[0.0]
    tf = Real[55.0]
    ...
    # for _ in 1:1
        ...
        (x, u, dx, fx, t0, tf) = solve_NLP(ns, nx, nu, n, x, cx, u, cu, bx, bu, c_eq, c_le, b_eq, b_le, f, na, a, f_obj, na_obj, a_obj, f_int, na_int, a_int, τ, t0, tf, ct, bt, D, W, 1e-6)
...
function option(index, ns, n, x, u, τ, tf, t0)
    ...
    plot_graph(index, plot_x, plot_u, plot_t, tf, t0)
end
...
function plot_graph(index, plot_x, plot_u, plot_t, tf, t0)

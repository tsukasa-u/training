...
function compute_coeff!(coeff::struct_coefficients, w::tuple_w, r::tuple_r, Q::struct_Q, μ::Vector{T}) where T<:Real
    ...
    r.rp[:] .= c(w.x, w.u) + w.y
    r.rd[:] .= w.s.*w.y - μ
    r.rhat[:] .= w.s.*r.rp - r.rd

    # eye_uss = diagm([1.0 for _ in  1:(w.nu + 2*w.ns)])
    # P = [Q.Quu transpose(Q.Qsu) zero_us; Q.Qsu zero_ss I; zero_su Y _S]
    # ret = -inv(P+ 1E-3*eye_uss)*[Q.Qu transpose(Q.Qxu); r.rp Q.Qsx; r.rd zero_sx]

    # coeff.α .= vec(ret[1:w.nu, 1])
    # coeff.η .= vec(ret[(w.nu + 1):(w.nu + w.ns), 1])
    # coeff.χ .= vec(ret[(w.nu + w.ns + 1):end, 1])

    # coeff.β .= ret[1:w.nu, 2:end]
    # coeff.θ .= ret[(w.nu + 1):(w.nu + w.ns), 2:end]
    # coeff.ζ .= ret[(w.nu + w.ns + 1):end, 2:end]
#---------------------------------------------------

    Y_1::Matrix{T} = diagm([1/ele for ele in w.y])
    SY_1::Matrix{T} = diagm(w.s./w.y)

    # _R = Q.Quu + transpose(Q.Qsu)*SY_1*Q.Qsu
    # ret = ones(w.nu, w.nx)
    # if size(_R) == (1, 1)
    #     # R = cholesky(_R)
    #     R = cholesky(Hermitian(_R))
    #     # LinearAlgebra.issuccess(R)
    #     # @assert LinearAlgebra.issuccess(R) "Cholesky factorization failed"
    #     ret = -R.U\R.L\[Q.Qu + transpose(Q.Qsu)*Y_1*r.rhat transpose(Q.Qxu) + transpose(Q.Qsu)*SY_1*Q.Qsx]
    # else
    #     ret = -_R[1, 1]\[Q.Qu + transpose(Q.Qsu)*Y_1*r.rhat transpose(Q.Qxu) + transpose(Q.Qsu)*SY_1*Q.Qsx]
    # end
    ...
    coeff.α[:] .= -R\(Q.Qu + transpose(Q.Qsu)*Y_1*r.rhat)
    coeff.β[:, :] .= -R\(transpose(Q.Qxu) + transpose(Q.Qsu)*SY_1*Q.Qsx)

    coeff.η[:] .= Y_1*r.rhat + SY_1*Q.Qsu*coeff.α
    coeff.θ[:, :] .= SY_1*(Q.Qsx + Q.Qsu*coeff.β)
    # coeff.η[:] .= -transpose(Q.Qsu)\(Q.Qu + Q.Quu*coeff.α)
    # coeff.θ[:, :] .= -transpose(Q.Qsu)\(transpose(Q.Qxu) + Q.Quu*coeff.β)

    coeff.χ[:] .= -r.rp - Q.Qsu*coeff.α
    coeff.ζ[:, :] .= -Q.Qsx - Q.Qsu*coeff.β
    #---------------------------------------------------

    # P = [Q.Quu transpose(Q.Qsu) zero_us; Q.Qsu zero_ss Matrix(I, w.ns, w.ns); zero_su Y _S]
    # # println(size(P))
    # # println(size(Q.Qu), " : ", size(Q.Qxu), " : ", size(Q.Qsx), " : ", size(zero_sx), " : ", size(r.rp), " : ", size(r.rd))
    # R = [Q.Qu transpose(Q.Qxu); r.rp Q.Qsx; r.rd zero_sx]
    # C = [coeff.α coeff.β; coeff.η coeff.θ; coeff.χ coeff.ζ]

    # # println(P*C + R)
    # x_label = vcat(["1"], ["x$i" for i in 1:w.nx])
    # y_label = vcat(["u$i" for i in 1:w.nu], ["s$i" for i in 1:w.ns], ["y$i" for i in 1:w.ns])
    # plots_h = heatmap(x_label, y_label, P*C + R)
    # plots_h = heatmap(Q.Quu*coeff.α+transpose(Q.Qsu)*coeff.η+Q.Qu)
    # plots_h = heatmap(Q.Qsu*coeff.α+coeff.χ+r.rp)
    # plots_h = heatmap(Y*coeff.η+_S*coeff.χ+r.rd)
    
    # frame(heatmap_anim, plots_h)
    #---------------------------------------------------
...
function FFP!(nw::S, list_w::array_w{T, S}, list_coeff::Array{struct_coefficients{T, S}, 1}) where {S<:Integer, T<:Real}
    ...
    if isfailed
        ...
        # for i in failed_step:nw-1
            ...
            list_w.new[i].s[:] = list_w.old[i].s[:]
            list_w.new[i].y[:] = list_w.old[i].y[:]
            list_w.new[i].u[:] = list_w.old[i].u[:]
            list_w.new[i + 1].x[:] = list_w.old[i + 1].x[:]
        end
    end
...
function loop!(n::S, nw::S, list_w::array_w{T, S}, list_r::Array{tuple_r{T, S}, 1}, list_coeff::Array{struct_coefficients{T, S}, 1}, μ::Vector{T}) where {S<:Integer, T<:Real}
    ...
    for k in 1:n
        ...
        vec1 = Vector{T}[[0 for i in 1:nw], [abs(list_coeff[i].α[1]) > list_coeff[i].α[1]/abs(list_coeff[i].α[1]) ? 1 : list_coeff[i].α[1] for i in 1:nw]]
        vec2 = Vector{T}[[0 for i in 1:nw], [abs(list_coeff[i].β[1]) > list_coeff[i].β[1]/abs(list_coeff[i].β[1]) ? 1 : list_coeff[i].β[1] for i in 1:nw]]
...
function init_xu!(list_w::array_w{T, S}, bx::Array{T, 1}, bu::Array{Array{T, 1}, 1}) where {S<:Integer, T<:Real}
    bx0 = bx
    bu0 = bu[1]
    ...
    list_w.new[1].u .= bu0
    list_w.old[1].u .= bu0
    for i in 2:list_w.nw
        list_w.new[i].x .= f(list_w.new[i-1].x, list_w.new[i-1].u)
        list_w.old[i].x .= f(list_w.new[i-1].x, list_w.new[i-1].u)
        list_w.new[i].u .= bu[i]
        list_w.old[i].u .= bu[i]
    end
end
...
function main()
    ...
    init_xu!(list_w, [-10, 0.7, 0.0], [[-cos(i*0.017-0.10) > 0.0 ? 0.12 : -0.12] for i in 1:nw])
...
function plot_graph(index, plot_x, plot_u, plot_s, plot_y, plot_t; vec1::Array{Array{T, 1}, 1}=[T[],T[]], vec2::Array{Array{T, 1}, 1}=[T[],T[]]) where T<:Real
    ...
    plot!([-5.5 + (1  )*cos(i) for i in 0:0.01:6.28], [-1.0 + (1  )*sin(i) for i in 0:0.01:6.28])
    plot!([-8.0 + (0.5)*cos(i) for i in 0:0.01:6.28], [ 0.2 + (0.5)*sin(i) for i in 0:0.01:6.28])
    plot!([-2.5 + (1.5)*cos(i) for i in 0:0.01:6.28], [ 1.0 + (1.5)*sin(i) for i in 0:0.01:6.28])
...
# gif(heatmap_anim, "cos.gif", fps=10)